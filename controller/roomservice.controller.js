...
const ROOMSERVICEModel = require('../model/RoomService');

const uploadROOMSERVICE = async (req, res) => {
    try {
        const file = req.file;
        ...
        const scriptPath = path.resolve(__dirname, "../script/process_roomservice.py");
        ...
        child.on('close', async (code) => {
            ...
            try {
                ...
                const Document = new ROOMSERVICEModel({
...
const deleteROOMSERVICE = async (req, res) => {
    try {
        ...
        const fileToDelete = await ROOMSERVICEModel.findById(fileId);
        if (!fileToDelete) {
            return res.status(404).json({ success: false, msg: "File not found." });
        }

        await ROOMSERVICEModel.findByIdAndDelete(fileId);
...
const getROOMSERVICE = async (req, res) => {
    try {
        const files = await ROOMSERVICEModel.find().select("-data");
...
module.exports = { uploadROOMSERVICE, deleteROOMSERVICE , getROOMSERVICE};
