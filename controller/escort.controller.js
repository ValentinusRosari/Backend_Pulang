...
const ESCORTModel = require('../model/Escort');
require("dotenv").config();

const uploadESCORT = async (req, res) => {
    try {
        const file = req.file;
        ...
        const scriptPath = path.resolve(__dirname, "../script/process_escort.py");
        ...
        child.on('close', async (code) => {
            ...
            try {
                ...
                const Document = new ESCORTModel({
...
const deleteESCORT = async (req, res) => {
    try {
        const { fileId } = req.params;

        const fileToDelete = await ESCORTModel.findById(fileId);
        if (!fileToDelete) {
            return res.status(404).json({ success: false, msg: "File not found." });
        }

        await ESCORTModel.findByIdAndDelete(fileId);
