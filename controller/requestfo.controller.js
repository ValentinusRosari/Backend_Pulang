...
const REQUESTModel = require('../model/RequestFO');
...
const uploadREQUEST = async (req, res) => {
    try {
        ...
        const scriptPath = path.resolve(__dirname, "../script/process_request.py");
        ...
        child.on('close', async (code) => {
            ...
            try {
                ...
                const Document = new REQUESTModel({
...
const deleteREQUEST = async (req, res) => {
    try {
        ...
        const fileToDelete = await REQUESTModel.findById(fileId);
        if (!fileToDelete) {
            return res.status(404).json({ success: false, msg: "File not found." });
        }

        await REQUESTModel.findByIdAndDelete(fileId);
...
const getREQUEST = async (req, res) => {
    try {
        const files = await REQUESTModel.find().select("-data");
...
module.exports = { uploadREQUEST, deleteREQUEST, getREQUEST };
