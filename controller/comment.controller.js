...
const uploadCOMMENT = async (req, res) => {
    try {
        ...
        const scriptPath = path.resolve(__dirname, "../script/process_comment.py");
...
const deleteCOMMENT = async (req, res) => {
    try {
        ...
        const fileToDelete = await COMMENTModel.findById(fileId);
        if (!fileToDelete) {
            return res.status(404).json({ success: false, msg: "File not found." });
        }

        await COMMENTModel.findByIdAndDelete(fileId);
...
const getCOMMENT = async (req, res) => {
    try {
        const files = await COMMENTModel.find().select("-data");
...
module.exports = { uploadCOMMENT, deleteCOMMENT, getCOMMENT };
